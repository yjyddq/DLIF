...
from utils.utils import save_checkpoint, AverageMeter, Logger, accuracy, mkdirs, adjust_learning_rate, time_to_str, adjust_learning_rate_restart
...
def train():
    ...
    get_dataset_train(config.protocol, config.src1_data, config.src2_data, config.src3_data, config.tgt_data, config.batch_size, config.test_batch_size)
    ...
    if config.optim == 'SGD':
        optimizer_u = optim.SGD(optimizer_u_dict, lr=config.init_lr, momentum=config.momentum, weight_decay=config.weight_decay)
        optimizer_v = optim.SGD(optimizer_v_dict, lr=config.init_lr, momentum=config.momentum, weight_decay=config.weight_decay)
    ...
    for iter_num in range(1,max_iter + 1):
        ...
        fake_cluster_label_2 = torch.arange(input1_fake_shape + 1, input1_fake_shape + input2_fake_shape + 1).view(-1,1).cuda()
        fake_cluster_label_3 = torch.arange(input1_fake_shape + input2_fake_shape + 1,input1_fake_shape + input2_fake_shape + input3_fake_shape + 1).view(-1,1).cuda()
        source_cluster_label = torch.cat([real_cluster_label_1, real_cluster_label_2, real_cluster_label_3,
                                          fake_cluster_label_1, fake_cluster_label_2, fake_cluster_label_3], dim=0).view(-1)

        aaic_u_loss = criterion["supcontra"](torch.cat([fu_q_norm.unsqueeze(1), fu_k_normH.unsqueeze(1),fu_k_normL.unsqueeze(1)],dim=1),source_cluster_label)
        aaic_v_loss = criterion["supcontra"](torch.cat([fv_q_norm.unsqueeze(1), fv_k_normH.unsqueeze(1), fv_k_normL.unsqueeze(1)],dim=1),source_id_label)
        ...
        id_amb_label = torch.FloatTensor(input_data.size(0), config.global_ID_num).fill_(1/config.global_ID_num).cuda()
        ...
        if local_rank == 0:
            ...
            print(
                ...
                % (
                    ...
                    loss_classifier.avg, classifer_top1.avg, loss_aaicu.avg, loss_identity.avg, loss_orthogonal.avg, loss_aaicv.avg,
        ...
        if (iter_num % iter_per_epoch == 0 and local_rank ==0):
            ...
            valid_args, log_prob_filename = eval(tgt_valid_dataloader, u_encoder, classifier, criterion["amsoftmax"], config, epoch, local_rank)
            ...
            final_result_file = config.logs +  'epoch_{}/'.format(epoch) + "{}_best_performance.txt".format(config.protocol)
            ...
            save_checkpoint(save_list, is_best, u_encoder, config.gpus, config.checkpoint_path, config.best_model_path, filename='uencoder')
            save_checkpoint(save_list, is_best, classifier, config.gpus, config.checkpoint_path, config.best_model_path, filename='cls')
            save_checkpoint(save_list, is_best, v_encoder, config.gpus, config.checkpoint_path, config.best_model_path, filename='vencoder')
            save_checkpoint(save_list, is_best, discriminator, config.gpus, config.checkpoint_path, config.best_model_path,filename='dis')
            print('\r', end='', flush=True)
            log.write(
        ...
        adjust_learning_rate(optimizer_u, epoch, init_param_lr_u, config.final_lr, config.step, config.gamma, config.warmup_epochs)
        adjust_learning_rate(optimizer_v, epoch, init_param_lr_v, config.final_lr, config.step, config.gamma, config.warmup_epochs)
