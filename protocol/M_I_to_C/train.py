...
from dataset.get_loader import get_dataset, get_dataset_wid, get_dataset_2to1_wid
from models.module import US_Encoder,VS_Encoder,Classifier,Discriminator, CLUB
from loss.hard_triplet_loss import HardTripletLoss
from loss.losses import OrthogonalLoss,AMSoftmaxLoss,SimilarityLoss_Cos,SupConLoss,Contraloss
...
def train():
    ...
    # load data
    src1_train_dataloader, src2_train_dataloader,tgt_valid_dataloader =\
    get_dataset_2to1_wid(config.protocol, config.src1_data, config.src2_data, config.tgt_data, config.batch_size, config.test_batch_size)
    ...
    for iter_num in range(1, max_iter + 1):
        ...
        input_data = torch.cat([src1_img_real, src2_img_real,
                                src1_img_fake, src2_img_fake,], dim=0)

        source_label = torch.cat([src1_label_real, src2_label_real,
                                  src1_label_fake, src2_label_fake,], dim=0)

        source_id_label = torch.cat([src1_id_real, src2_id_real,
                                     src1_id_fake, src2_id_fake], dim=0)
        ...
        source_cluster_label = torch.cat([real_cluster_label_1, real_cluster_label_2,
                                          fake_cluster_label_1, fake_cluster_label_2],dim=0).view(-1)
